...
namespace SmartLife.Client.Merchant
{
    public partial class frmLogin : Form
    {
        ...
        { 
            ...
            System.TimeSpan TS = EndDate.Subtract(StartDate);
            var currentCalendar = System.Threading.Thread.CurrentThread.CurrentCulture.Calendar;

            long d = (long)((currentCalendar.GetYear(EndDate) - currentCalendar.GetYear(StartDate)) * 365 + (currentCalendar.GetDayOfYear(EndDate) - currentCalendar.GetDayOfYear(StartDate)));
...
#else
            HttpAdapter.postAsyncAsJSON(authEndPoint + "/AuthenticateMerchant", new AuthMerchantParam { StationCode = userCode, PasswordHash = MD5Provider.Generate(password) }.ToStringObjectDictionary(), new { ApplicationId = Common.APPLICATION_ID }.ToStringObjectDictionary(), (ret, res) =>
            {

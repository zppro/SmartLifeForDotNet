...
namespace SmartLife.CertManage.ManageServices.Pub
{
    ...
    public class DeviceService : BaseWcfService
    {
        #region CMS标准接口
        ...
        #region 自定义
        #region EasyUIgrid数据格式的列表 OrgDeviceListByPage
        [WebInvoke(UriTemplate = "OrgDeviceListByPage", Method = "POST", RequestFormat = WebMessageFormat.Json, ResponseFormat = WebMessageFormat.Json, BodyStyle = WebMessageBodyStyle.Bare)]
        ...
        #region 查询列表 QueryOrgDevice
        [WebGet(UriTemplate = "QueryOrgDevice?parms={strParms}", ResponseFormat = WebMessageFormat.Json)]
        public CollectionInvokeResult<StringObjectDictionary> QueryOrgDevice(string strParms)
        {
            CollectionInvokeResult<StringObjectDictionary> result = new CollectionInvokeResult<StringObjectDictionary> { Success = true };
            try
            {
                var dictionary = new StringObjectDictionary().MixInJson(strParms);
                result.rows = BuilderFactory.DefaultBulder().ListStringObjectDictionary("OrgDeviceList", dictionary);
            }
            catch (Exception ex)
            {
                result.Success = false;

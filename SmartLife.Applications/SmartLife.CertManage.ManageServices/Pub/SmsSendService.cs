...
{ 
    ...
    public class SmsSendService : AppBaseWcfService
    {
        ...
        public EasyUIgridCollectionInvokeResult<SmsSend> ListForEasyUIgridPage(EasyUIgridCollectionParam<SmsSend> param)
        {
            EasyUIgridCollectionInvokeResult<SmsSend> result = new EasyUIgridCollectionInvokeResult<SmsSend> { Success = true };
            try
            {
                /***********************begin 复杂查询添加代码*********************/
                StringObjectDictionary filters = new StringObjectDictionary();
                ...
                if (param.filterFields != null)
                {
                    foreach (var field in param.filterFields)
                    {
                        ...
                        if (field.key.Equals("CheckInTime_Start") && DateTime.TryParse(field.value, out parseTime))
                        {
                            whereClause.Add(string.Format("CheckInTime >= '{0}' ", field.value));
                        }
                        else if (field.key.Equals("CheckInTime_End") && DateTime.TryParse(field.value, out parseTime))
                        {
                            whereClause.Add(string.Format("CheckInTime <= '{0}' ", field.value));
                        }
                        else
                ...
                gridCollectionPager.EasyUIgridDoPage4Model<SmsSend>(filters, param, ref result);
            }
            catch (Exception ex)
            {
                result.Success = false;

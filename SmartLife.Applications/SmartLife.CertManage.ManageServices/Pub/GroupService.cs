...
namespace SmartLife.CertManage.ManageServices.Pub
{
    ...
    public class GroupService : AppBaseWcfService
    {
        ...
        #region 评估系统独立部署   用户角色授权
        [WebInvoke(UriTemplate = "ListForEasyUIgrid2", Method = "POST", RequestFormat = WebMessageFormat.Json, ResponseFormat = WebMessageFormat.Json, BodyStyle = WebMessageBodyStyle.Bare)]
        public EasyUIgridCollectionInvokeResult<Group> ListForEasyUIgrid2(EasyUIgridCollectionParam<Group> param)
        {
            EasyUIgridCollectionInvokeResult<Group> result = new EasyUIgridCollectionInvokeResult<Group> { Success = true };
            try
            {

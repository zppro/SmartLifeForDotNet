...
namespace SmartLife.CertManage.ManageServices.Oca
{
    ...
    public class MerchantService : BaseWcfService
    {
        #region CMS标准接口
        ...
        public ModelInvokeResult<MerchantPK> Nullify(string strStationId)
        {
            ModelInvokeResult<MerchantPK> result = new ModelInvokeResult<MerchantPK> { Success = true };
            try
            {
                ...
                Merchant merchant = new Merchant { StationId = _StationId };
                /***********************begin 自定义代码*******************/
                merchant.OperatedBy = NormalSession.UserId.ToGuid();
                merchant.OperatedOn = DateTime.Now;
                merchant.DataSource = GlobalManager.DIKey_00012_ManualEdit;
        ...
        public InvokeResult NullifySelected(string strStationIds)
        {
            ...
            try
            {
                ...
                string statementName = new Merchant().GetUpdateMethodName();
                foreach (string strStationId in arrStationIds)
                {
                    Merchant merchant = new Merchant { StationId = strStationId.ToGuid() };
                    /***********************begin 自定义代码*******************/
                    merchant.OperatedBy = NormalSession.UserId.ToGuid();
                    merchant.OperatedOn = DateTime.Now;
                    merchant.DataSource = GlobalManager.DIKey_00012_ManualEdit;
                    /***********************end 自定义代码*********************/
                    statements.Add(new IBatisNetBatchStatement { StatementName = statementName, ParameterObject = merchant.ToStringObjectDictionary(false), Type = SqlExecuteType.UPDATE });
                }
                BuilderFactory.DefaultBulder().ExecuteNativeSqlNoneQuery(statements);
            }
            catch (Exception ex)
        ...
        private void DeleteCascade(List<IBatisNetBatchStatement> statements,MerchantPK pk)
        {
            //此处增加级联删除代码
        }
        #endregion

...
namespace SmartLife.CertManage.ManageServices.Sys
{
    ...
    public class TreeService : AppBaseWcfService
    {
        ...
        [WebGet(UriTemplate = "OutputSQL/{strTreeIds}", ResponseFormat = WebMessageFormat.Json)]
        public CollectionInvokeResult<string> OutputSQL(string strTreeIds)
        {
            CollectionInvokeResult<string> result = new CollectionInvokeResult<string> { Success = true };
            try
            {

                string[] treeIds = strTreeIds.Split("|".ToCharArray(), StringSplitOptions.RemoveEmptyEntries);
                string tableName = treeIds[0];
                string tableNameItem = treeIds[1];
                string treeIdAll = "";
                if (treeIds.Length > 2)
                {
                    ...
                    treeIdAll = treeIdAll.Remove(treeIdAll.LastIndexOf(","), 1);
                    treeIdAll = "where Status='1' and TreeId in (" + treeIdAll + ")";
                }
                else
                {
                    treeIdAll = "";
                ...
                _rows.AddRange(BuilderFactory.DefaultBulder().ExecuteSPForQuery<StringObjectDictionary>("SP_Tol_UspOutputDataEx", new { TableName = tableName, WhereClause = treeIdAll }).Select(item => item["targetscript"].ToString()).ToList());
                _rows.AddRange(BuilderFactory.DefaultBulder().ExecuteSPForQuery<StringObjectDictionary>("SP_Tol_UspOutputDataEx", new { TableName = tableNameItem, WhereClause = treeIdAll }).Select(item => item["targetscript"].ToString()).ToList());
                result.rows = _rows;
            }
            catch (Exception ex)
            {

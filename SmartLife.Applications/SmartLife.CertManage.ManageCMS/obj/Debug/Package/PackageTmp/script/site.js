    ...
    $("body").bind("ajaxSuccess", function (e, jhr, ajaxOptions) {
        ...
        if (ajaxOptions.dataType == "json") {
            ...
            if (jhr.status == 403) {
                ...
                alertError('您已长时间没有活动，请重新登录', function () {
                    if (top.onSessionOut) {
                        top.onSessionOut();
                    }
                    else {
...
function Redirect(url, fromTab) {
    ...
    if (fromTab) {
        if (parent.execMode == "CS" || parent.execMode == "ForSeat") {
        ...
        if (window.execMode == "CS" || window.execMode == "ForSeat") {
...
function Call_OpenNewWinAsCallServiceFromIndex(url, callServiceId) {
    top.external.OpenNewWinAsCallServiceFromIndex(url, callServiceId);
}

function OpenNewWinAsCallService(url, callServiceId,iType) {
    var str;
    getTo(callservice_InvokeUrl + '/' + callServiceId, null, function (ret) {
        str = $.toJSON(ret.instance);
        if (str) {
            if (iType == "info") {
                top.external.OpenNewWinAsCallServiceInfo(url, str);
            }
            else {
                top.external.OpenNewWinAsCallService(url, str);
            }
        }
    });
} 

function Call_OpenNewWinAsWorkOrderFromIndex(url,workOrderId) {
    top.external.OpenNewWinAsWorkOrderFromIndex(url, workOrderId);
}
 
function setExecModeToCS() {
    window.execMode = "CS";
}

function setEnvironmentVar(jsonStr) {
    if (jsonStr) {
        var environmentVar = eval('(' + jsonStr + ')');
        window.gUserId = environmentVar.UserId;
        window.gUserCode = environmentVar.UserCode;
        window.gUserName = environmentVar.UserName;
        window.gExtId = environmentVar.ExtId;
        window.gExtCode = environmentVar.ExtCode;
        window.gIPCCDial = environmentVar.IPCCDial;
        window.gCallCenterId = environmentVar.CallCenterId;
        window.gAreaId = environmentVar.AreaId;
    }
}

function setDialBack(callServiceId, oldManId, callNo, name) {
    window.external.DialBackNoWin(callServiceId, oldManId, callNo, name);
}

function setServiceTypeUrl(jsonStr) {
    if (jsonStr) {
        window.serviceTypeData = eval('(' + jsonStr + ')');
    }
}

function setPageData(jsonStr) {
    if (jsonStr) {
        window.pageData = eval('(' + jsonStr + ')');
    }
}

function refreshFromExternal() {
    if (window.currentMenuId && window.currentMenuCode) { 
        if (window.frames["tab_" + window.currentMenuId]) { 
            var model = window.frames["tab_" + currentMenuId].models[window.currentMenuCode];
            if (model && model.gridId) {
                window.frames["tab_" + window.currentMenuId].$('#' + model.gridId).datagrid('reload');
            }
        }
    }
}

function refreshWorkOrderFromIndex() {
    if (window.currentMenuId && window.currentMenuCode) {
        if (window.frames["tab_" + window.currentMenuId]) {
            if (window.currentMenuCode == "B1800") {
                window.frames["tab_" + window.currentMenuId].RefreshAll();
            }
        }
    }
}
...
function easyuiLoader(param, success, error) {
    ...
    var fn_BeforeSend = null;
    if (opts.headers) {
        fn_BeforeSend = function (jqXHR, s) {
            for (var key in opts.headers) {
                jqXHR.setRequestHeader(key, opts.headers[key]);
            }
        }
    }
...
//身份证位数不够时  会补空格
function easyuigrid_IDNo(val, row, idx) {
    val = val.toString().replace('unescape', '');
    val = val.toString().replace(/\'|\(|\)/g, '');
    //        val = val.replace(/\'/g,'');替换单引号
    //        val = val.replace(/\(/g,'');左括号
    //        val = val.replace(/\)/g,'');右括号
    do {
        val = val.replace('%20', ' ');
...
function easyuigrid_dateFormatter(val, row, idx) {
    if (val == null || val == "") return "";
    if (val.indexOf("Date") < 0) { var date = new Date(val); return date.toString(this.datefmt); };
...
function easyuigrid_ageFormatter3(val, row, idx) {
    var strIDNo = row.IDNo.toString().match(/(\d{15}$)|(\d{17}([0-9]|(X|x)))/g); 
    if (!strIDNo || strIDNo=='null') return "";
    strIDNo = strIDNo.toString();
    if (strIDNo.length != 18 && strIDNo.length != 15) return "";
    var dateStr;
    if (strIDNo.length == 18) {
        dateStr = strIDNo.substr(6, 8);
    }
    else {
        dateStr = "19" + strIDNo.substr(6, 6);
    }
...
function easyuigrid_statusFormatter(val, row, idx) {
    return val == "1" ? "正常" : (val == "0" ? "已作废" : "--");
}
...
function getTreeData(treeCode, orderByClause, treeParams, d, headers) {
...
function buildTree(type, treeContainerId, treeSettings, nodes) {
    ...
    if (type == "zTree") {
        ...
        if (isAync) {
            ...
            setting.data = __getTreeNode(nodes, '_');
        }
        ...
        if (isAync) {
            ...
            if (treeSettings.needRoot) {
                setting.data = [{ id: '_', text: treeSettings.rootName || '无', state: 'open', children: __getTreeNode(nodes, '_')}]; // __getTreeNode(nodes, '_');
            }
            else {
                setting.data = __getTreeNode(nodes, '_');
            }
...
function initTree(type, treeContainerId, treeSettings, treeData, selectedOrCheckedNodes, d) {
    ...
    if (_.isFunction(treeData)) {
        treeData(function (nodes) {
            var treeObj = buildTree(type, treeContainerId, treeSettings, nodes);
...
function refreshTree(type, treeContainerId, treeCode, orderByClause, treeParams) {
    ...
    getTreeData(treeCode, orderByClause, treeParams, function (nodes) {
        if (type == "easyUITree") {
            var selectedNodeOld = $('#' + treeContainerId).tree('getSelected');
            $('#' + treeContainerId).tree('loadData', __getTreeNode(nodes, '_'));
            ...
            $('#' + treeContainerId).combotree('loadData', __getTreeNode(nodes, '_'));
        }
        else if (type == "zTree") {
...
function findTreeNode(treeObj, level, index) {
    if (treeObj) {
        var cLevel = 1;
        var roots = treeObj.tree('getRoots');
        return _findTreeNode(treeObj, roots, 1, level, index);
    }
    return null;
}

function _findTreeNode(treeObj, nodes, clevel, level, index) {
    if (clevel == level) {
        return __findTreeNode(nodes, index);
    }
    else if (clevel < level) {
        var newNodes = [];
        for (var i = 0; i < nodes.length; i++) {
            newNodes = _.union(newNodes, treeObj.tree('getChildren', nodes[i].target));
        }
        return _findTreeNode(treeObj, newNodes, clevel + 1, level, index);
    }
    return null;
}

function __findTreeNode(nodes, index) {
...
function __getTreeNode(nodes, parentId) {
    ...
    if (rets.length > 0) {
        rets = _.map(rets, function (o) {
            ...
            var children = __getTreeNode(sons, o.id);
            var state = 'open';
            if (o.isParent) {
                state = 'closed';
            }
...
function nullifyTo(url, success, options, headers) {
    var defaults = {
        type: 'PUT',
        url: url,
        success: success,
...
function nullifyTo2(url, selected, done, headers) {
    var ajax = nullifyTo(url + 'NullifySelected/' + selected, null, null, headers);
    if (done) {
        ajax.done(function (ret) {
            if (ret.Success) {
                done.call(this, ret);
...
function stopTo(url, success, options, headers) {
    var defaults = {
        type: 'PUT',
        url: url,
        success: success,
...
function stopTo2(url, selected, done, headers) {
    var ajax = nullifyTo(url + 'StopSelected/' + selected, null, null, headers);
    if (done) {
        ajax.done(function (ret) {
            if (ret.Success) {
                done.call(this, ret);
...
function restartTo(url, success, options, headers) {
    var defaults = {
        type: 'PUT',
        url: url,
        success: success,
...
function restartTo2(url, selected, done, headers) {
    var ajax = nullifyTo(url + 'RestartSelected/' + selected, null, null, headers);
    if (done) {
        ajax.done(function (ret) {
            if (ret.Success) {
                done.call(this, ret);
...
function saveTo(url, data, done, headers) { 
    var ajax;
    if (data.isCreate) {
        ajax = postTo(url, $.toJSON(data), null, null, headers);
    }
    else {
        ajax = putTo(url + data.PK, $.toJSON(data), null, null, headers);
    }
    if (done) {
        ajax.done(function (ret) {
            if (ret.Success) {
...
function IsNull(vars) {
    return vars == null ? "0" : vars;
}

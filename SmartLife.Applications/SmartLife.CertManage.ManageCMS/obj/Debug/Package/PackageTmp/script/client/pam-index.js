    ...
    if (!gToken) {
        ...
        getTo(checkIsLoggedIn_InvokeUrl, null, function (result) {
            if (!result.ret.LoggedIn) {
                ...
                dialogUrlPrefix = (window.execMode == 'ForPam' ? '/' : '');
                gUserInfo = { isSuperAdmin: false, isAgencyUser: false };
                    ...
                    getTo(baseCMSInvokeUrl + '/Pub/UserService/GetMappingAgencys', null, function (r2) {
                        ...
                        if (window.execMode == 'ForPam') {
                            ...
                            if (r2.rows && r2.rows.length > 0) {
                                window.external.SetClientEnvironment(r2.rows[0].StationId);
                            }
                        }
                    }, { async: false }, { isSuperAdmin: gUserInfo.isSuperAdmin });
                }, { async: false });
            }
        });
    }

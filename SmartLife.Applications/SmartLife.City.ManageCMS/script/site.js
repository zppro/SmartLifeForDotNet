$(function () {
    ...
    $("body").bind("ajaxSuccess", function (e, jhr, ajaxOptions) {
        ...
        if (ajaxOptions.dataType == "json") { 
            top.$('.datagrid-mask').hide();
            top.$('.datagrid-mask-msg').hide();
...
function initTree(type, treeContainerId, treeSettings, treeData, selectedOrCheckedNodes, d, localAync, nameField) {
    ...
    if (_.isFunction(treeData)) {
        treeData(function (nodes) {
            ...
            if (nodes.length > 0) {
                if (selectedOrCheckedNodes) {
                    var cLevel = selectedOrCheckedNodes.replace(/^(\d+)\^(\d+)$/, '$1');
                    var cOrder = selectedOrCheckedNodes.replace(/^(\d+)\^(\d+)$/, '$2');

                    if (isNumber(cLevel) && isNumber(cOrder)) {
                        cLevel = parseInt(cLevel);
                        cOrder = parseInt(cOrder) - 1;
                        if (type == "zTree") {
                        }
                        else {
                            var levelNodes = _.where(nodes, { levels: cLevel });
                            if (levelNodes.length > 0) {
                                var node = treeObj.tree('find', levelNodes[cOrder].id);
                                if (node) {
                    ...
                    if (type == "zTree") {
                        ...
                        if (selectedOrCheckedNodes != null) {
                            var node = treeObj.tree('find', nodes[0].id);
                            treeObj.tree('expandTo', node.target);
                            treeObj.tree('select', node.target);
                        }
                        else {

...
function Call_OpenNewWinAsCallServiceInfo(url, callServiceId) {
    var str;
...
function setDialBack(callServiceId, callNo, name) {
    window.external.DialBackNoWin(callServiceId, callNo, name);
}
...
function refreshFromExternal() {
    if (window.currentMenuCode) {
        if (window.frames["tab_" + window.currentMenuCode]) {
            var model = window.frames["tab_" + currentMenuCode].models[window.currentMenuCode];
            if (model && model.gridId) {
                window.frames["tab_" + window.currentMenuCode].$('#' + model.gridId).datagrid('reload');
            }
        }
    }
}
    ...
    if (window.currentMenuCode) {
        if (window.frames["tab_" + window.currentMenuCode]) {
            if (window.currentMenuCode == "B1800") {
                window.frames["tab_" + window.currentMenuCode].RefreshAll();
            }
        }
    }
}
...
function easyuigrid_ageFormatter3(val, row, idx) {
    if (row.IDNo.length != 18) return "";
    var dateStr = row.IDNo.substr(6, 8);
...
function alertInfo(msg, fn) {
    $.messager.alert('消息提示', msg, 'info').window({ onClose: function () {
        if (fn) {
            fn();
        }
    }
...
function alertError(msg, fn) {
    $.messager.alert('错误提示', msg, 'error').window({ onClose: function () {
        if (fn) {
            fn();
        }
    }
...
function nullifyTo2(url, selected, done) {
    var ajax = nullifyTo(url + 'NullifySelected/' + selected);
    if (done) {
        ajax.done(function (ret) {
            if (ret.Success) {
                done.call(this, ret);

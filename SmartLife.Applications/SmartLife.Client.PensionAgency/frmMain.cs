...
namespace SmartLife.Client.PensionAgency
{
    ...
    public partial class frmMain : Form
    {
        ...
        public void Ready4OpenTarget(string pageKey, string pageText, string pageUrl, string pageOpenTarget, string pageCode)
        {
        ...
        {
            PensionAgencyVar.StationId = Guid.Parse(stationId);
        }
        ...
        {
            Control ctl = null;
            for (int index = 0; index < PensionAgencyVar.AuthorizedModules.Count; index++)
            {
                dynamic module = PensionAgencyVar.AuthorizedModules[index];
        ...
        private void ChooseTab(string tabPageKey, string tabPageText, string tabPageUrl, string tabOpenTarget)
        {
            ...
            else
            {
                ...
                wb.DocumentCompleted += new WebBrowserDocumentCompletedEventHandler((object o, WebBrowserDocumentCompletedEventArgs we) =>
                {
                    ...
                    if (we.Url.ToString() == PensionAgencyVar.AccessPoint + "/index.htm")
                    {
                        wb.Document.InvokeScript("setExecModeForPam");
                        wb.Document.InvokeScript("loadPage", new object[] { tabPageUrl });
                    }
                    else if (we.Url.ToString() == PensionAgencyVar.AccessOfProvice + "/index.htm")
                    {
                        wb.Document.InvokeScript("setExecModeForPam");
                        wb.Document.InvokeScript("loadPage", new object[] { tabPageUrl });
                    }
                ...
                {
                    wb.Url = new Uri(PensionAgencyVar.AccessPoint + "/index.htm");
                }
                else if (tabOpenTarget == "_tab_province")
                {
                    wb.Url = new Uri(PensionAgencyVar.AccessOfProvice + "/index.htm");
                }
            }
        }
        ...
        private void wbProvinceSum_DocumentCompleted(object sender, WebBrowserDocumentCompletedEventArgs e)
        {
            ...
            else if (e.Url.ToString() == PensionAgencyVar.AccessOfProvice + "/index.htm")
            {

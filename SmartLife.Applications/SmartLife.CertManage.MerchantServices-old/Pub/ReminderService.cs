    ...
    public class ReminderService : BaseWcfService
    {
        ...
        public InvokeResult IgnoreReminder(IgnoreReminderParam param)
        {
            ...
            try
            {
                ...
                BuilderFactory.DefaultBulder().ExecuteNativeSqlNoneQuery(new IBatisNetBatchStatement { StatementName = "IgnoreReminderByMerchant", ParameterObject = p, Type = SqlExecuteType.UPDATE });
            }
            catch (Exception ex)
            {
                result.Success = false;
